...
import model.DentistaCarlosCT;
import model.Equipo_DentalCarlosCT;
...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	private JComboBox<DentistaCarlosCT> cboDentistas;
	...
	public DlgEquipoDental() {
		...
		cboDentistas = new JComboBox<DentistaCarlosCT>();
	...
	void cargarDentistas() {
		...
		try {
			String jpql = "select d from DentistaCarlosCT d";
			List<DentistaCarlosCT> lstd = manager.createQuery(jpql, DentistaCarlosCT.class).getResultList();
			for (DentistaCarlosCT dentista : lstd) {
	...
	void listar() {
		EntityManager manager = JPAUtil.getEntityManager();
		String jpql = "select e, d.nombres, d.correo, es.titulo from Equipo_DentalCarlosCT e join e.dentista d join d.especialidad es order by 1 asc";

		try {
			List<Object[]> resultados = manager.createQuery(jpql, Object[].class).getResultList();

			for (Object[] fila : resultados) {
				Equipo_DentalCarlosCT e = (Equipo_DentalCarlosCT) fila[0];
				String d = (String) fila[1];
				String correo = (String) fila[2];
				String es = (String) fila[3];

				imprimir("Número de equipo     : " + e.getNequipo());
				imprimir("Nombre equipo        : " + e.getNombre());
				imprimir("Costo                : " + e.getCosto());
				imprimir("Fecha de adquisición : " + e.getFecha());
				imprimir("Dentista             : " + d + " - " + correo);
				imprimir("Especialidad         : " + es);
	...
	void buscar() {
		EntityManager manager = JPAUtil.getEntityManager();
		String codigo = txtNroEquipo.getText();

		try {
			if (codigo == null || codigo.isEmpty()) {
				mensajeInfo("Introdusca un codigo");
				return;
			}

			int id = Integer.parseInt(codigo);
			Equipo_DentalCarlosCT e = manager.find(Equipo_DentalCarlosCT.class, id);

			if (e == null) {
				mensajeInfo("No se encontro la solicitud");
				return;
			}
			txtNombre.setText(e.getNombre());
			txtCosto.setText(String.valueOf(e.getCosto()));
			cboEstados.setSelectedItem(e.getEstado());
			txtFechaAdquisicion.setText(e.getFecha().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
	...
	void modificar() {
		...
		try {
			...
			e.setDentista((DentistaCarlosCT) cboDentistas.getSelectedItem());
			e.setEstado((String) cboEstados.getSelectedItem());
			...
			mensajeInfo("Se actualizo con exito");
	...
	void eliminar() {
		...
		try {
			...
			Equipo_DentalCarlosCT s = manager.find(Equipo_DentalCarlosCT.class, id);
			
			manager.getTransaction().begin();
			manager.remove(s);
			manager.getTransaction().commit();
			
			mensajeInfo("Se elimino con exito");
